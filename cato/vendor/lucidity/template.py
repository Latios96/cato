...
class Template(object):
    ...
    def expanded_pattern(self):
        ...
        """
        return self._TEMPLATE_REFERENCE_REGEX.sub(self._expand_reference, self.pattern)
    ...
    def _expand_reference(self, match):
        ...
        if self.template_resolver is None:
            raise lucidity.error.ResolveError(
                "Failed to resolve reference {0!r} as no template resolver set.".format(
                    reference
                )
            )
        ...
        if template is None:
            raise lucidity.error.ResolveError(
                "Failed to resolve reference {0!r} using template resolver.".format(
                    reference
                )
            )
                    ...
                    if key in parsed:
                        if parsed[key] != value:
                            raise lucidity.error.ParseError(
                                ...
                                "{0!r} detected. Values were {1!r} and {2!r}.".format(
    ...
    def format(self, data):
        ...
        return self._PLAIN_PLACEHOLDER_REGEX.sub(
            functools.partial(self._format, data=data), format_specification
        )
    ...
    def _format(self, match, data):
        ...
        try:
            ...
            raise lucidity.error.FormatError(
                "Could not format data {0!r} due to missing key {1!r}.".format(
                    data, placeholder
                )
            )
    ...
    def references(self):
        ...
        format_specification = self._construct_format_specification(self.pattern)
    ...
    def _construct_regular_expression(self, pattern):
        ...
        expression = re.sub(
            ...
            functools.partial(self._convert, placeholder_count=defaultdict(int)),
            expression,
        )
