...
class EndMessageGenerator:
    ...
    def generate_end_message(self, result: TestSuiteExecutionResult)->str:
        ...
        total_tests_succeded = len(list(filter(lambda x: x.result == TestStatus.SUCCESS, all_tests)))
        total_tests_failed = len(list(filter(lambda x: x.result == TestStatus.FAILED, all_tests)))

        return emoji.emojize(
"""Result:
Ran {} tests
{}  succeded :white_check_mark:
{}  failed   :x:""".format(total_tests, total_tests_succeded, total_tests_failed), use_aliases=True)