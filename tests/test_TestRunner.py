...
def test_should_replace_placeholder():
    ...
    command_runner = mock_safe(CommandRunner)
    test_runner = TestRunner(command_runner, reporter)
    test = Test(name="my first test", command="crayg -s {test_resources}/test.json -o {image_output_png}")
    ...
    reporter.report_start_test.assert_called_with(test)
    command_runner.run.assert_called_with("crayg -s {}/test.json -o my first test.png".format(os.path.join('test', 'suite', 'my first test')))